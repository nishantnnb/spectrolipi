...
#${MODAL_ID} { position: fixed; inset: 0; display: none; align-items: center; justify-content: center; z-index: 2147483600; background: rgba(0,0,0,0.45); -webkit-backdrop-filter: blur(3px); backdrop-filter: blur(3px);} 
  #${MODAL_ID} .card { width: 96%; max-width: 760px; max-height: 90vh; overflow: auto; background: #0f1216; color: #fff; border-radius: 10px; border: 1px solid rgba(255,255,255,0.08); box-shadow: 0 12px 36px rgba(0,0,0,0.35); padding: 14px; font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial; }
  ...
  <div id="scc-presets-area" style="margin-bottom:8px;display:none;background:#071018;padding:8px;border-radius:6px;border:1px solid rgba(255,255,255,0.03)"></div>

        <div class="grid">
          <div class="row">
            <label for="scc-thresh">Threshold (0-1)</label>
            <input id="scc-thresh" type="number" min="0" max="1" step="0.01" value="0.70">
          </div>
          <div class="row">
            <label for="scc-quality">Quality</label>
            <select id="scc-quality">
              <option value="fast">Fast (more downsampling)</option>
              <option value="balanced" selected>Balanced</option>
              <option value="accurate">Accurate</option>
            </select>
          </div>
          <div class="row">
            <label for="scc-pitch">Pitch tolerance (%)</label>
            <input id="scc-pitch" type="number" min="0" max="50" step="1" value="5">
          </div>
          <div class="row">
            <label for="scc-pitch-steps">Pitch steps (odd)</label>
            <input id="scc-pitch-steps" type="number" min="1" max="11" step="2" value="5">
          </div>
          <div class="row">
            <label for="scc-time">Time-stretch tolerance (%)</label>
            <input id="scc-time" type="number" min="0" max="50" step="1" value="0">
          </div>
          <div class="row">
            <label for="scc-time-steps">Time steps (odd)</label>
            <input id="scc-time-steps" type="number" min="1" max="11" step="2" value="1">
          </div>
          <div class="row">
            <label for="scc-composite">Composite templates</label>
            <select id="scc-composite">
              <option value="none" selected>None (run each template)</option>
              <option value="mean">Mean</option>
              <option value="median">Median</option>
            </select>
          </div>
          <div class="row">
            <label for="scc-energy">Energy floor percentile (0-100)</label>
            <input id="scc-energy" type="number" min="0" max="100" step="1" value="5">
          </div>
          <div class="row">
        ...
        <div class="actions">
          <label style="display:flex;align-items:center;gap:6px;margin-right:auto;font-size:13px;color:#cbd5e1">
            <input type="checkbox" id="scc-merge" />
            <span>Merge detections</span>
          ...
          <button class="btn" id="scc-run">Run</button>
          <button class="btn" id="scc-cancel" style="background:#374151;">Cancel</button>
        </div>
      </div>`;
  ...
  function createScanOverlay(parentEl, totalSegments) {
    try {
      ...
      box.style.background = 'rgba(6,8,10,0.65)';
      ...
      const cx = 70, cy = 70, rOut = 60, rIn = 40;
      const segs = [];
      ...
      label.className = 'scc-scan-label';
      label.style.color = '#cbd5e1';
      label.style.fontSize = '13px';
      label.style.marginTop = '8px';
      label.textContent = `Scanning 0 / ${totalSegments}`;
      ...
      return {
        markDone(i) { try { if (segs[i]) segs[i].setAttribute('fill','#10b981'); label.textContent = `Scanning ${i+1} / ${segs.length}`;} catch (e) {} },
        setStatus(txt) { try { label.textContent = String(txt||''); } catch (e) {} },
  ...
  const modal = _showModal();
      ...
      // ensure cancel also aborts any in-flight scan
      cancel.onclick = () => { try { if (__scc_currentScan) __scc_currentScan.cancelled = true; } catch (e) {} close(); };
      ...
      html += '<div style="margin-top:8px; display:flex; align-items:center; justify-content:flex-end; gap:12px">'
        + '<label style="margin-right:auto; display:flex; align-items:center; gap:6px; font-size:13px; color:#cbd5e1">'
        + '<input type="checkbox" id="scc-merge-apply" />'
        + '<span>Merge detections</span>'
        + '<input id="scc-merge-gap-apply" type="number" min="0" max="10" step="0.1" value="1.0" style="width:68px;background:#0b1523;border:1px solid rgba(255,255,255,0.08);color:#e5e7eb;padding:4px 6px;border-radius:4px">'
        + '<span>sec apart</span>'
        + '</label>'
        ...
        + '</div>';
            ...
            radios.forEach(rb => rb.addEventListener('change', (ev) => {
              ...
              $('scc-thresh').value = String(selPreset.opts.threshold);
              $('scc-quality').value = String(selPreset.opts.quality);
              $('scc-pitch').value = String(selPreset.opts.pitchTolPct);
              $('scc-pitch-steps').value = String(selPreset.opts.pitchSteps);
              $('scc-time').value = String(selPreset.opts.timeTolPct);
              $('scc-time-steps').value = String(selPreset.opts.timeSteps);
              $('scc-composite').value = String(selPreset.opts.composite||'none');
              $('scc-energy').value = String(selPreset.opts.energyPct);
              $('scc-mindur').value = String(selPreset.opts.minDurMs);
              $('scc-minfreq').value = String(Math.max(0, Math.floor(selPreset.opts.minFreq||0)));
              $('scc-maxfreq').value = String(Math.max(0, Math.ceil(selPreset.opts.maxFreq||0)));
      ...
      run.onclick = async () => {
        // Validate selection: must contain only rows checked as SCC Template
        try {
          const selRows = getGridSelectedTemplates();
          const onlySccTemplates = Array.isArray(selRows) && selRows.length > 0 && selRows.every(r => !!r.sccTemplate);
          if (!onlySccTemplates) { alert('Process can be applied only on SCC template(s). Pls deselect other rows.'); return; }
        } catch(e) {}
        const opts = {
          threshold: clamp(Number(($('scc-thresh').value||'0.7')), 0, 1),
          quality: String(($('scc-quality').value||'balanced')),
          pitchTolPct: Math.max(0, Number(($('scc-pitch').value||'0'))||0),
          pitchSteps: Math.max(1, Number(($('scc-pitch-steps').value||'1'))||1),
          timeTolPct: Math.max(0, Number(($('scc-time').value||'0'))||0),
          timeSteps: Math.max(1, Number(($('scc-time-steps').value||'1'))||1),
          composite: String(($('scc-composite').value||'none')),
          energyPct: clamp(Number(($('scc-energy').value||'5')), 0, 100),
          minDurMs: Math.max(0, Number(($('scc-mindur').value||'0'))||0),
          minFreq: Math.max(0, Number(($('scc-minfreq').value||'0'))||0),
          maxFreq: Math.max(0, Number(($('scc-maxfreq').value||'0'))||0)
        };
        // If the user previously selected a preset from the scan results, merge that
        // preset's options into the run options so Run uses the same tuning (coarseSX,
        // coarseSY, coarseThresh, maxCandidates, maxRefine, etc.). Preset values take
        // precedence. However, preserve explicit form values when present.
        try {
          if (typeof __scc_lastSelectedPresetIndex === 'number' && Array.isArray(__scc_lastScanRows)) {
            const pi = __scc_lastSelectedPresetIndex;
            const presetRow = __scc_lastScanRows[pi];
            if (presetRow && presetRow.opts) {
              const p = presetRow.opts;
              // Merge only backend tuning keys so user's edited form values (front settings)
              // continue to take effect. Backend keys include coarseSX, coarseSY, coarseThresh,
              // maxCandidates, maxRefine, and allowLowVar.
              const backendKeys = ['coarseSX','coarseSY','coarseThresh','maxCandidates','maxRefine','allowLowVar'];
              backendKeys.forEach(k => { if (Object.prototype.hasOwnProperty.call(p, k)) { try { opts[k] = p[k]; } catch(e){} } });
            }
            // The preset scan used allowLowVar during trial; if not explicitly present in preset opts,
            // enable allowLowVar so the run mirrors the scan's relaxed variant acceptance.
            if (opts.allowLowVar === undefined) opts.allowLowVar = true;
          }
        } catch (e) {}
        try {
          const result = await runSccWithOptions(opts);
          // If the run added 0 rows but the user previously ran a preset scan that predicted >0,
          // offer to re-run with a relaxed low-variance skip so results align with the scan.
          try {
            const predicted = (__scc_lastScanRows && typeof __scc_lastSelectedPresetIndex === 'number') ? (__scc_lastScanRows[__scc_lastSelectedPresetIndex] && __scc_lastScanRows[__scc_lastSelectedPresetIndex].count) || 0 : 0;
            if ((result && result.added === 0) && predicted > 0) {
              // If we have cached detections from the preset scan, apply them automatically
              // to avoid asking the user to re-run or making them wait for another expensive run.
              try {
                const presetIdx = __scc_lastSelectedPresetIndex;
                const presetRow = (Array.isArray(__scc_lastScanRows) && typeof presetIdx === 'number') ? __scc_lastScanRows[presetIdx] : null;
                const cached = presetRow && Array.isArray(presetRow.detections) ? presetRow.detections : null;
                if (cached && cached.length) {
                  // Apply cached detections same as Apply button logic
                  try { window.__spectroWait && window.__spectroWait.show({ etaText: 'Applying preset (cached)…' }); } catch(e){}
                  try { await new Promise(r=>setTimeout(r,20)); } catch(e){}
                  const selRows = getGridSelectedTemplates();
                  const speciesGuess = (function(){ try { const uniq = Array.from(new Set((selRows||[]).map(r => (r.species||'').trim()))).filter(Boolean); if (uniq.length === 1) return uniq[0]; const spLbl = document.getElementById('speciesResult'); return spLbl ? (spLbl.textContent||'').trim() : ''; } catch(e){ return ''; } })();
                  const runNumber2 = (++__scc_runCounter);
                  let nextId2 = nextIdFromGrid();
                  // Optionally merge cached detections by gap < 1s
                  let cachedToUse = cached;
                  try {
                    const mergeFlag = !!(document.getElementById('scc-merge') && document.getElementById('scc-merge').checked) || !!(document.getElementById('scc-merge-apply') && document.getElementById('scc-merge-apply').checked);
                    let gapSec = 1.0; try { const elA = document.getElementById('scc-merge-gap-apply'); const elM = document.getElementById('scc-merge-gap'); const v = parseFloat(elA && elA.value || elM && elM.value); if (!isNaN(v)) gapSec = Math.max(0, v); } catch(e){}
                    if (mergeFlag) cachedToUse = mergeDetectionsByGap(cached, gapSec);
                  } catch(e){}
                  // Determine scientific name for the species guess (same logic as used by run)
                  const speciesScientificGuess = (function(){
                    try {
                  ...
                  const rowsToAdd = cachedToUse.map((d,i) => ({
                    id: nextId2 + i,
                    Selection: String(nextId2 + i),
                    beginTime: Number(round4(d.t1)),
                    endTime: Number(round4(d.t2)),
                    lowFreq: Number(round4(d.f1)),
                    highFreq: Number(round4(d.f2)),
                  ...
                  if (rowsToAdd.length) {
                    ...
                    try {
                      const grid = window.annotationGrid;
                      if (grid && typeof grid.getSelectedRows === 'function') {
                        const prev = grid.getSelectedRows(); if (Array.isArray(prev)) prev.forEach(rObj => { try { if (typeof rObj.deselect === 'function') rObj.deselect(); else if (typeof grid.deselectRow === 'function') grid.deselectRow(rObj); } catch(e){} });
                      }
                      if (grid && typeof grid.selectRow === 'function') { for (const r of rowsToAdd) { try { grid.selectRow(r.id); } catch(e){} } }
                    } catch(e){}
                  }
                  try { window.__spectroWait && window.__spectroWait.hide(); } catch(e){}
                  try { alert(`${rowsToAdd.length} detections applied from preset scan (cached).`); } catch(e){}
                } else {
                  // No cached detections available — fall back to offering a relaxed re-run
                  const doRelax = confirm(`Run produced 0 detections but preset scan predicted ${predicted}.\nRe-run now with relaxed low-variance filtering to attempt to recover similar detections?`);
                  if (doRelax) {
                    opts.allowLowVar = true;
                    const result2 = await runSccWithOptions(opts);
                    if (result2 && result2.added) alert(`${result2.added} detections added after relaxed run.`);
                    else alert('No detections added after relaxed run.');
                  }
                }
              } catch (e) {}
            }
          ...
          alert('SCC failed: ' + (e && e.message ? e.message : String(e)));
        }
      };
    });
    btn.__sccWired = true;
