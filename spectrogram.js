...
function fft(real, imag) {
  ...
  if ((1 << levels) !== n) throw new Error('FFT must be power of two');
  for (let i=0; i<n; i++) { 
...
self.onmessage = function(e) {
  const { mono, fftSize, hop, startFrame, numFrames } = e.data;
  const N = fftSize;
  const bins = N / 2;
  ...
  for (let fIdx = 0; fIdx < numFrames; fIdx++) {
    const localOff = fIdx * hop;
    for (let n = 0; n < N; n++) { 
      const s = mono[localOff + n] || 0; 
    ...
    for (let b = 0; b < bins; b++) { 
      ...
      const db = 20 * log10(mag + 1e-12); 
      if (db < minDB) minDB = db; if (db > maxDB) maxDB = db; 
      spectra[fIdx * bins + b] = mag; 
    }
  }
  self.postMessage({ spectra, minDB, maxDB }, [spectra.buffer]);
};`;
  ...
  try {
    ...
    console.warn('[Spectrogram] Failed to create worker Blob URL', e);
  }
  ...
  function onSpectrogramPointerMove(ev) {
    ...
    try {
      ...
      const freq = Math.max(ymin, Math.min(ymax, ymax - (yViewport / Math.max(1, imgH)) * (ymax - ymin)));
  ...
  function drawXTicksAxis(sampleRate, totalFrames, imgW, imgH, framesPerSec, pxpf, visibleStartSec = 0){
    ...
    drawYAxis(sampleRate, imgH, globalThis._spectroYMax, globalThis._spectroYMin || 0);
  }
  ...
  function updateXTicksFromScroll() {
    if (!globalThis._spectroImageWidth || !globalThis._spectroFramesPerSec || !globalThis._spectroPxPerFrame) {
      drawXTicksAxis(globalThis._spectroSampleRate || 44100, globalThis._spectroNumFrames || 0, globalThis._spectroImageWidth || 800, globalThis._spectroImageHeight || IMAGE_H, globalThis._spectroFramesPerSec || (globalThis._spectroSampleRate/(getFFTSize()/2)), globalThis._spectroPxPerFrame || 2, 0, globalThis._spectroYMin || 0);
      return;
    }
  ...
  function drawYAxis(sampleRate, imgH, ymax, ymin = 0){
    if(!axisCtx) return;
    ...
    const bottomFreq = (typeof ymin === 'number' && ymin >= 0) ? Math.min(topFreq - 1, ymin) : 0;
    const yTicks = 6;
    axisCtx.textAlign = 'right'; axisCtx.textBaseline = 'middle';
    for(let i=0;i<yTicks;i++){
      const t = i / (yTicks - 1);
      const freq = topFreq - t * (topFreq - bottomFreq);
  ...
  function drawAxisPlaceholder(){ if(!axisCtx) return; axisCtx.fillStyle='#000'; axisCtx.fillRect(0,0,axisCanvas.width, axisCanvas.height); axisCtx.fillStyle='#111'; axisCtx.fillRect(0, AXIS_TOP, axisCanvas.width, IMAGE_H); drawYAxis(44100, IMAGE_H, 22050, 0); }
  function drawSpectrogramPlaceholder(){ const placeholderW = Math.max(300, (scrollArea && scrollArea.clientWidth) ? scrollArea.clientWidth : 400); const innerW = placeholderW; canvas.width = innerW; canvas.height = AXIS_TOP + IMAGE_H + AXIS_BOTTOM; canvas.style.width = innerW + 'px'; canvas.style.height = VIEWPORT_H + 'px'; canvas.style.background = 'transparent'; ctx.clearRect(0,0,canvas.width,canvas.height); ctx.fillStyle='#111'; ctx.fillRect(0, AXIS_TOP, placeholderW, IMAGE_H); updateXTicksFromScroll(); }
  ...
  async function processFile(file, fftSize, overlapFactor, pxpf, cmap){
    ...
    const spectroStartTime = performance.now();
    let usedCores = 1;
    ...
    if (numFrames > 100) {
      ...
      try {
        ...
        for (let i = 0; i < numCores; i++) {
          ...
          const worker = new Worker(_workerBlobUrl || 'spectro_worker.js');
          workers.push(worker);
        ...
        console.error("[Spectrogram] Worker execution failed:", err);
    ...
    const spectroEndTime = performance.now();
    console.log(`[Spectrogram Performance] Generated spectrogram in ${((spectroEndTime - spectroStartTime) / 1000).toFixed(2)} seconds using ${usedCores} core(s).`);
  ...
  const imageW = Math.max(1, numFrames * pxpf); // intrinsic spectrogram pixel width (unscaled)
    const imageH = globalThis._spectroImageHeight || IMAGE_H;
  ...
  ctx.fillRect(0, Math.round(AXIS_TOP * mainScaleY), Math.round(imageW * mainScaleX * displayScaleX), Math.round(imageH * mainScaleY));
    ...
    const initialYMin = 0;
    globalThis._spectroYMin = initialYMin;
    const safeDenom = denom || 1e-12;

    // Precompute Y-axis mappings (Frequency doesn't change over time)
    const yMapIdxA = new Int32Array(imageH);
    const yMapIdxB = new Int32Array(imageH);
    const yMapFrac = new Float32Array(imageH);
    for (let y = 0; y < imageH; y++) {
      const ty = y / (imageH - 1);
      const freq = initialYMax - ty * (initialYMax - initialYMin);
      const fracBin = (freq / nyqDecoded) * (bins - 1);
      const fIdx = Math.floor(fracBin);
      yMapIdxA[y] = Math.max(0, Math.min(bins - 1, fIdx));
      yMapIdxB[y] = Math.max(0, Math.min(bins - 1, fIdx + 1));
      yMapFrac[y] = fracBin - fIdx;
    }
    ...
    for (let tileX = 0, tileIndex = 0; tileX < imageW; tileX += tileW, tileIndex++){
      ...
      for (let localX = 0; localX < w; localX++){
        ...
        const frameBaseIdx = baseFrame * bins;
        for (let y = 0; y < imageH; y++){
          const a = spectra[frameBaseIdx + yMapIdxA[y]];
          const b = spectra[frameBaseIdx + yMapIdxB[y]];
          const mag = a + (b - a) * yMapFrac[y];
          const magAdj = mag * gain;
          const db = 20 * Math.log10(magAdj + 1e-12);
          let v = (db - bottom) / safeDenom; 
          if (v < 0) v = 0; else if (v > 1) v = 1; else if (v !== v) v = 0;
		  //v = Math.pow(v, 1.5);
          const lutIdx = (v * 255) | 0;
  ...
  const dw = Math.max(1, Math.round(w * mainScaleX * displayScaleX));
        ...
        if (!firstTilePainted) {
          ...
          try {
            ...
            // Bypass event listener race conditions: Ingest XC annotations synchronously right before the UI unlocks
            if (globalThis._pendingXcAnnotations) {
              if (globalThis._annotations && typeof globalThis._annotations.addMany === 'function') {
                globalThis._annotations.addMany(globalThis._pendingXcAnnotations, 'import');
              }
              globalThis._pendingXcAnnotations = null;
            }
    ...
    alignCanvasLeft();
    drawYAxis(sr, imageH, globalThis._spectroYMax, globalThis._spectroYMin || 0);
  ...
  async function reRenderFromSpectra(ymax, ymin = 0){
    ...
    let tempYMin = Number(ymin) || 0;
    if (tempYMin >= ymaxClamped) tempYMin = ymaxClamped - 100;
    const yminClamped = Math.max(0, tempYMin);
    globalThis._spectroYMax = ymaxClamped;
    globalThis._spectroYMin = yminClamped;
    ...
    for (let y = 0; y < imageH; y++) {
      const ty = y / (imageH - 1);
      const freq = ymaxClamped - ty * (ymaxClamped - yminClamped);
  ...
  drawYAxis(sr, imageH, ymaxClamped, yminClamped);
  updateXTicksFromScroll();
  try { drawViewportFromTiles && drawViewportFromTiles(); } catch(e) {}
    try {
      if (scrollArea) {
  ...
  function _rebuildTileBitmapSync(tile, ymaxHz, lut, gain){
    ...
    const yminClamped = globalThis._spectroYMin || 0;
  ...
  async function _recolorVisibleTiles(ymaxHz){
    ...
    globalThis._spectroYMax = ymaxHz;
    try { drawViewportFromTiles && drawViewportFromTiles(); } catch(e) {}
  }
  ...
  const debouncedLiveApply = debounce(async ()=>{
    try {
      ...
      if (globalThis._spectroSpectra) {
        ...
        const useYMin = globalThis._spectroYMin || 0;
        ...
        if (globalThis._spectroImageWidth && globalThis._spectroImageWidth > LARGE_IMAGE_W) {
          ...
          await reRenderFromSpectra(useY, useYMin);
        }
        updateXTicksFromScroll();
      ...
      const h = globalThis._spectroImageHeight || IMAGE_H;
      const lut = buildLUT(cmapSelect.value || 'custom');
      const gain = Math.max(0.0001, parseFloat(gainInput && gainInput.value) || 1);
  ...
  if (gainInput) {
    gainInput.addEventListener('input', ()=>{ updateGainLabel(); debouncedLiveApply(); });
  ...
  // --- Dynamic Resize Handler for canvas stretch ---
  const handleDynamicResize = debounce(() => {
    if (!globalThis._spectroSpectra) {
      drawSpectrogramPlaceholder();
      drawAxisPlaceholder();
      return;
    }
    const intrinsicW = globalThis._spectroImageIntrinsicWidth || 0;
    const scrollEl = scrollArea || document.getElementById('scrollArea');
    if (scrollEl && canvas && intrinsicW > 0) {
      const newViewportW = scrollEl.clientWidth;
      const metrics = _computeDisplayMetrics(intrinsicW, newViewportW);
      _applyDisplayMetricsFromComputed(metrics);
      
      const displayedCssWidth = metrics.displayed;
      canvas.style.width = Math.max(1, displayedCssWidth) + 'px';
      
      const spacer = document.getElementById('spectroSpacer');
      if (spacer) {
        spacer.style.width = Math.max(1, displayedCssWidth) + 'px';
      }
      ...
      _scheduleAnnotationOverlaySync('resize');
    }
  }, 150);

  window.addEventListener('resize', handleDynamicResize, { passive: true });

  // --- Y-Axis Interactive Zoom & Pan ---
  if (axisCanvas) {
    let isYDragging = false;
    let startDragY = 0;
    let startYMin = 0;
    let startYMax = 0;
    let yAxisUpdateTimer = null;
    let pendingYMin = null;
    let pendingYMax = null;
    let isYAxisUpdating = false;

    async function applyYAxisUpdate() {
      if (pendingYMin === null || pendingYMax === null) {
        isYAxisUpdating = false;
        return;
      }
      isYAxisUpdating = true;
      const targetMin = pendingYMin;
      const targetMax = pendingYMax;
      pendingYMin = null;
      pendingYMax = null;
      try {
        if (typeof globalThis._spectrogram_reRenderFromSpectra === 'function') {
          await globalThis._spectrogram_reRenderFromSpectra(targetMax, targetMin);
          const yZoomSelect = document.getElementById('yZoomSelect');
          if (yZoomSelect && targetMin === 0) {
            const closest = Array.from(yZoomSelect.options).reduce((prev, curr) => Math.abs(curr.value - targetMax) < Math.abs(prev.value - targetMax) ? curr : prev);
            yZoomSelect.value = closest.value;
          }
        }
      ...
      isYAxisUpdating = false;
      // If the mouse moved while we were rendering, immediately trigger the next frame
      if (pendingYMin !== null) {
        requestAnimationFrame(applyYAxisUpdate);
      }
    }

    function scheduleYAxisUpdate(newYMin, newYMax) {
      pendingYMin = newYMin;
      pendingYMax = newYMax;
      
      // Fast-path: instantly update the visual Y-axis ticks and annotation overlays for 60fps responsiveness
      try {
        globalThis._spectroYMin = newYMin;
        globalThis._spectroYMax = newYMax;
        const sr = globalThis._spectroSampleRate || 44100;
        const imgH = globalThis._spectroImageHeight || IMAGE_H;
        drawYAxis(sr, imgH, newYMax, newYMin);
        if (typeof window.renderAllAnnotations === 'function') window.renderAllAnnotations();
        if (typeof window.renderSelectionOverlay === 'function') window.renderSelectionOverlay();
        if (globalThis._displayAnnotationLabels && typeof globalThis._displayAnnotationLabels.sync === 'function') globalThis._displayAnnotationLabels.sync();
        if (canvas) canvas.style.transform = ''; // Ensure any lingering transform is cleared
      } catch(e) {}

      // Fire render instantly if not already running
      if (!isYAxisUpdating) {
        applyYAxisUpdate();
      }
    }
    ...
    axisCanvas.addEventListener('pointermove', (e) => {
      ...
      const imgH = globalThis._spectroImageHeight || IMAGE_H;
      if (!isYDragging) { axisCanvas.style.cursor = (y >= AXIS_TOP && y <= AXIS_TOP + imgH) ? 'ns-resize' : 'default'; return; }
      const dy = y - startDragY;
      const span = startYMax - startYMin;
      const dfreq = (dy / imgH) * span;
      let newYMax = startYMax + dfreq;
      let newYMin = startYMin + dfreq;
      const nyq = globalThis._spectroOriginalNyquist || (globalThis._spectroSampleRate ? globalThis._spectroSampleRate / 2 : 22050);
      if (newYMax > nyq) { newYMax = nyq; newYMin = newYMax - span; }
      if (newYMin < 0) { newYMin = 0; newYMax = newYMin + span; }
      scheduleYAxisUpdate(Math.max(0, newYMin), Math.min(nyq, newYMax));
    });

    axisCanvas.addEventListener('pointerdown', (e) => {
      if (!globalThis._spectroSpectra || e.button !== 0) return;
      const rect = axisCanvas.getBoundingClientRect();
      const y = e.clientY - rect.top;
      const imgH = globalThis._spectroImageHeight || IMAGE_H;
      ...
      startDragY = y;
      const nyq = globalThis._spectroOriginalNyquist || (globalThis._spectroSampleRate ? globalThis._spectroSampleRate / 2 : 22050);
      startYMin = globalThis._spectroYMin || 0;
      startYMax = globalThis._spectroYMax || nyq;
      try { axisCanvas.setPointerCapture(e.pointerId); } catch(err){}
    ...
    const stopDrag = (e) => {
      ...
      isYDragging = false;
      try { axisCanvas.releasePointerCapture(e.pointerId); } catch(err){}
      axisCanvas.style.cursor = 'ns-resize';
    };
    axisCanvas.addEventListener('pointerup', stopDrag);
    axisCanvas.addEventListener('pointercancel', stopDrag);
    ...
    axisCanvas.addEventListener('wheel', (e) => {
      ...
      if (y < AXIS_TOP || y > AXIS_TOP + imgH) return;
      e.preventDefault();
      const ty = (y - AXIS_TOP) / imgH;
      const nyq = globalThis._spectroOriginalNyquist || (globalThis._spectroSampleRate ? globalThis._spectroSampleRate / 2 : 22050);
      let ymax = globalThis._spectroYMax || nyq;
      let ymin = globalThis._spectroYMin || 0;
      const span = ymax - ymin;
      const freqAtMouse = ymax - ty * span;
      let delta = e.deltaY;
      if (e.deltaMode === 1) delta *= 16;
      let newSpan = Math.max(100, Math.min(nyq, span * (delta > 0 ? 1.15 : (1 / 1.15))));
      let newYMax = freqAtMouse + ty * newSpan;
      let newYMin = freqAtMouse - (1 - ty) * newSpan;
      if (newYMax > nyq) { newYMax = nyq; newYMin = Math.max(0, newYMax - newSpan); }
      if (newYMin < 0) { newYMin = 0; newYMax = Math.min(nyq, newYMin + newSpan); }
      scheduleYAxisUpdate(newYMin, newYMax);
    }, { passive: false });

    axisCanvas.addEventListener('contextmenu', (e) => {
      if (!globalThis._spectroSpectra) return;
      const rect = axisCanvas.getBoundingClientRect();
      const y = e.clientY - rect.top;
      const imgH = globalThis._spectroImageHeight || IMAGE_H;
      ...
      scheduleYAxisUpdate(0, finalYMax);
    });
  }
...
(function(){
  ...
  try {
    globalThis._setSpectroLastSnapshot = function(snap) { 
      try { 
        lastSnapshot = snap; 
        if (undoBtn) {
          undoBtn.disabled = !lastSnapshot; 
          undoBtn.style.opacity = lastSnapshot ? '1.0' : '0.45';
          undoBtn.style.cursor = lastSnapshot ? 'pointer' : 'default';
        } 
      } catch(e){} 
    };
  ...
  function updateCutButtonEnabled(){
    ...
    const disableBase = !ready || playing || modalOpen;

    // Only enable Cut when spectrogram is ready, not playing, no modal open, and a selection exists
    cutBtn.disabled = disableBase || !hasSelection;
    // Select and Filters should be disabled when playing/not ready/modal open
    try {
      if (selectBtn) {
        const wasDisabled = selectBtn.disabled;
        selectBtn.disabled = disableBase;
    ...
    try { if (silenceBtn) silenceBtn.disabled = disableBase || !hasSelection; } catch(e){}
    try { if (filtersBtn) filtersBtn.disabled = disableBase; } catch(e){}
    try { if (normalizeBtn) normalizeBtn.disabled = disableBase; } catch(e){}
    try {
      const spectralCopyBtn = document.getElementById('spectralCopyBtn');
      if (spectralCopyBtn) spectralCopyBtn.disabled = disableBase;
    } catch(e){}
    // Undo should be available in both create and edit modes if there is a snapshot
    try { 
      if (undoBtn) {
        undoBtn.disabled = disableBase || !lastSnapshot; 
        undoBtn.style.opacity = (disableBase || !lastSnapshot) ? '0.45' : '1.0';
        undoBtn.style.cursor = (disableBase || !lastSnapshot) ? 'default' : 'pointer';
      } 
    } catch(e){}
  }
  ...
  async function _rebuildAllTilesFromSpectra(){
    ...
    for (let tileX=0, idx=0; tileX<imageW; tileX+=tileW, idx++){
      ...
      for (let localX=0; localX<w; localX++){
        ...
        const frameBaseIdx = frameIdx * bins;
        for (let y=0; y<imageH; y++){
          const a = spectra[frameBaseIdx + yMapIdxA[y]];
          const b = spectra[frameBaseIdx + yMapIdxB[y]];
          const mag = a + (b - a) * yMapFrac[y];
    ...
    try {
      ...
      for (let i=0;i<rows.length;i++){
        ...
        if (e <= c1Sec) {
          ...
          // spans across region
          if (c1Sec === c2Sec) {
            // If inserting exactly at a point inside the annotation, stretch it
            e = e + diff;
          } else {
            // If replacing a sub-region, trim to left portion
            e = c1Sec;
          }
        }
  ...
  function disarmCutMode(cancelled=false){
    ...
    hasSelection = false;
    try { globalThis._spectroCurrentSelection = null; globalThis._spectroSelectionAbsPx = null; } catch(e){}
  ...
  function onKeyDown(ev){ if (ev.key === 'Escape') { ev.preventDefault(); disarmCutMode(true); } }
  ...
  function armCutMode(){
    ...
    if (typeof globalThis.cancelStampMode === 'function') {
      try { globalThis.cancelStampMode(); } catch(e){}
    }
  ...
  glass.onpointerdown = (ev)=>{ ev.preventDefault(); if (ev.button===2) { // right-click: cancel selection mode
    ...
    glass.onpointerup = (ev)=>{
      if (!isDragging) return;
      const rect = glass.getBoundingClientRect();
      dragEndX = Math.max(0, Math.min(rect.width, ev.clientX - rect.left));
      // If the pointer moved only a few pixels and it was the primary button,
      // treat this as a single click (no-drag) and cancel selection (like right-click).
      try {
        const movePx = Math.abs(dragEndX - dragStartX);
        const CLICK_THRESHOLD_PX = 6;
        if ((ev.button === 0 || ev.button === undefined) && movePx <= CLICK_THRESHOLD_PX) {
          isDragging = false;
          try { disarmCutMode(true); updateCutButtonEnabled(); } catch(e){}
          try { if (glass.releasePointerCapture) glass.releasePointerCapture(ev.pointerId); } catch(e){}
          return;
        }
      } catch(e){}
      isDragging = false;
          try { glass.releasePointerCapture(ev.pointerId); } catch(e){}
      renderSelectionWithTimes();
    };
        glass.onpointercancel = (ev)=>{ 
          isDragging = false; 
          try { glass.releasePointerCapture(ev.pointerId); } catch(e){} 
        };
  ...
  const fps = globalThis._spectroFramesPerSec || 1; const i1 = Math.max(0, Math.floor(c1Sec * fps)); const i2 = Math.max(i1+1, Math.min(globalThis._spectroNumFrames||0, Math.ceil(c2Sec * fps)));
    ...
    try {
      if (window.__spectroWait) window.__spectroWait.show({ titleText: 'Cutting audio...', bodyText: 'Removing selected region and updating spectrogram. Please wait...' });
      await new Promise(r => setTimeout(r, 20));
      await cutSpectrogramRange(c1Sec, c2Sec);
      toast('Cut applied');
      if (undoBtn) { try { undoBtn.disabled = !lastSnapshot; } catch(_){} }
      // Clear the selection globals completely
      try { globalThis._spectroCurrentSelection = null; globalThis._spectroSelectionAbsPx = null; } catch(e){}
    } catch (e) {
      ...
      disarmCutMode(false); updateCutButtonEnabled(); }
  }
      ...
      try {
        if (window.__spectroWait) {
          window.__spectroWait.show({ 
            titleText: 'Undoing last edit...', 
            bodyText: 'Restoring previous state and updating spectrogram. Please wait...' 
          });
        }
        // Small delay to allow wait overlay layout to paint
        ...
        // Small delay to allow the restored canvas/tiles to paint before hiding the overlay
        await new Promise(r => setTimeout(r, 40));
        toast('Undo applied');
      } finally {
        lastSnapshot = null; 
  ...
  if (cutBtn) {
    cutBtn.addEventListener('click', (ev)=>{
      ...
      if (cutArmed) {
        if (window.confirm('Selected portion & overlapping annotations will be deleted. Continue?')) {
  ...
  window.addEventListener('modal-toggled', ()=>{ try { updateCutButtonEnabled(); } catch(e){} });
...
(function() {
        ...
        if (xcApiKey) {
          ...
          if (metadata && metadata.recordings && metadata.recordings.length > 0) {
            ...
            if (rec['annotation-set'] && Array.isArray(rec['annotation-set'].annotations)) {
              console.log("[XC Load] Found annotation-set in metadata:", rec['annotation-set'].annotations.length, "annotations");
              const pendingAnnos = rec['annotation-set'].annotations;
              globalThis._pendingXcAnnotations = pendingAnnos.map(a => {
  ...
  // Auto-load if URL contains ?XCID=... (or ?xc=..., ?id=..., etc.)
  function checkUrlAutoLoad() {
    const urlParams = new URLSearchParams(window.location.search);
    const xcIdParam = urlParams.get('XCID') || urlParams.get('xcid') || urlParams.get('xc') || urlParams.get('XC') || urlParams.get('id') || urlParams.get('xc_id') || urlParams.get('nr');
    if (xcIdParam) {
      // Trigger the load
  ...
  if (document.readyState === 'loading') {
    window.addEventListener('DOMContentLoaded', checkUrlAutoLoad);
  } else {
    checkUrlAutoLoad();
  }
})();
