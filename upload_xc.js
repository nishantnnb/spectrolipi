(function () {
  ...
  function buildModalDom() {
    ...
    .xc-grid { display:grid; grid-template-columns: repeat(3, 1fr); gap:12px; align-items:center; margin-bottom:16px; }
    .xc-row-full { grid-column:1 / -1; }
    .xc-label { font-size:13px; color:#333; margin-bottom:6px; display:block; font-weight: 500; }
    .xc-input, .xc-select { width:100%; box-sizing:border-box; padding:8px 10px; border:1px solid #d0d4d7; border-radius:6px; font-size:14px; background:#fff; color:#111; }
    ...
    const annotatorNameWrap = el('div', null,
      el('label', { cls: 'xc-label', html: 'Name of the Annotator:' }),
      el('input', { type: 'text', className: 'xc-input', id: 'xc-annname', placeholder: '', title: 'Optional for XC upload' })
    );
    const licenseWrap = el('div', null,
      el('label', { cls: 'xc-label', html: 'Annotation license:' }),
    ...
    const checkboxesRow = el('div', { style: 'display: flex; gap: 24px; align-items: center; margin-bottom: 10px;' });
    ...
    optionsWrap.appendChild(checkboxesRow);
    grid.appendChild(optionsWrap);

    const actions = el('div', { cls: 'xc-actions' });
...
(function(){
  ...
  function ensureSettingsModal() {
    ...
    backdrop.id = 'xcSettingsBackdrop';
      backdrop.style.cssText = 'display:none;position:fixed;inset:0;background:rgba(18,22,26,0.45);z-index:2147483651;align-items:center;justify-content:center;-webkit-backdrop-filter: blur(4px); backdrop-filter: blur(4px);';
    const modal = document.createElement('div');
    modal.id = 'xcSettingsModal';
    modal.setAttribute('role','dialog');
    modal.setAttribute('aria-modal','true');
      modal.style.cssText = 'width:420px;max-width:90%;background:#fff;color:#111;border-radius:10px;padding:18px;box-shadow:0 10px 40px rgba(0,0,0,0.25);font-family: system-ui, -apple-system, "Segoe UI", Roboto, Arial;';
    modal.innerHTML = [
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">',
        '<h3 style="margin:0;font-size:16px;">Xeno-canto Settings</h3>',
        '<button type="button" id="xcSettingsClose" aria-label="Close settings" style="background:transparent;border:0;color:#666;font-size:20px;line-height:1;cursor:pointer;">&times;</button>',
      '</div>',
        '<label for="xcApiKeyInput" style="display:block;font-size:13px;font-weight:500;margin-top:6px;color:#333;">API key</label>',
      '<div style="position:relative;display:flex;align-items:center;margin-top:4px;">',
        '<input id="xcApiKeyInput" type="password" autocomplete="off" spellcheck="false" style="width:100%;box-sizing:border-box;padding:8px 10px;padding-right:48px;border-radius:6px;border:1px solid #d0d4d7;background:#fff;color:#111;font-size:14px;" />',
        '<button type="button" id="xcApiToggleBtn" style="position:absolute;right:8px;background:transparent;border:none;color:#555;cursor:pointer;font-size:12px;padding:4px;">Show</button>',
      '</div>',
        '<label for="xcAnnotatorNameInput" style="display:block;font-size:13px;font-weight:500;margin-top:12px;color:#333;">Annotator display name</label>',
        '<input id="xcAnnotatorNameInput" type="text" autocomplete="off" style="width:100%;box-sizing:border-box;margin-top:4px;padding:8px 10px;border-radius:6px;border:1px solid #d0d4d7;background:#fff;color:#111;font-size:14px;" />',
        '<label for="xcDefaultLicenseInput" style="display:block;font-size:13px;font-weight:500;margin-top:12px;color:#333;">Default Annotation license</label>',
        '<select id="xcDefaultLicenseInput" style="width:100%;box-sizing:border-box;margin-top:4px;padding:8px 10px;border-radius:6px;border:1px solid #d0d4d7;background:#fff;color:#111;font-size:14px;">',
      '<option value=""></option>',
      '<option value="CC-BY-4.0">CC-BY-4.0</option>',
      '<option value="CC-BY-NC-4.0">CC-BY-NC-4.0</option>',
      '</select>',
        '<label for="xcEndpointInput" style="display:block;font-size:13px;font-weight:500;margin-top:12px;color:#333;">API endpoint</label>',
        '<input id="xcEndpointInput" type="text" style="width:100%;box-sizing:border-box;margin-top:4px;padding:8px 10px;border-radius:6px;border:1px solid #d0d4d7;background:#fff;color:#111;font-size:14px;" />',
        '<div id="xcSettingsStatus" style="min-height:18px;color:#059669;font-size:13px;margin-top:8px;"></div>',
        '<div style="display:flex;justify-content:flex-end;gap:10px;margin-top:16px;">',
        '<button type="button" id="xcSettingsCancel" style="padding:8px 12px;border-radius:8px;border:1px solid #e0e3e6;font-size:14px;cursor:pointer;background:#f6f7f8;color:#222;">Cancel</button>',
        '<button type="button" id="xcSettingsSave" style="padding:8px 12px;border-radius:8px;border:1px solid transparent;font-size:14px;cursor:pointer;background:#0b66ff;color:#fff;box-shadow:0 6px 18px rgba(11,102,255,0.12);">Save</button>',
      '</div>'
    ].join('');
    backdrop.appendChild(modal);
    document.body.appendChild(backdrop);

    // Wire the Show/Hide API Key Toggle
    const toggleBtn = document.getElementById('xcApiToggleBtn');
    const apiKeyInput = document.getElementById('xcApiKeyInput');
    ...
    function open() {
      ...
      document.getElementById('xcApiKeyInput').type = 'password';
      document.getElementById('xcApiToggleBtn').textContent = 'Show';
      document.getElementById('xcAnnotatorNameInput').value = s.annotatorName || '';
      document.getElementById('xcDefaultLicenseInput').value = s.license || '';
    ...
    function save() {
      ...
      const license = document.getElementById('xcDefaultLicenseInput').value.trim();
      ...
      if (!apiKey) {
        document.getElementById('xcSettingsStatus').textContent = 'API key is required.';
          document.getElementById('xcSettingsStatus').style.color = '#d32f2f';
        return;
      }
      persistSettings({ apiKey, annotatorName, license, endpoint });
