...
(function () {
  ...
  function annotationToRectPx(a) {
    ...
    const t1 = (ymaxHz - a.highFreq) / Math.max(1, ymaxHz - yminHz);
    const t2 = (ymaxHz - a.lowFreq) / Math.max(1, ymaxHz - yminHz);
  ...
  function getTrashIconRect(rectPx) {
    ...
    let top = Math.min(rectPx.top, rectPx.bottom);
    let bottom = Math.max(rectPx.top, rectPx.bottom);
    ...
    let x, y;
    if (w < TRASH_ICON_SIZE + 12 && visibleH < TRASH_ICON_SIZE + 12) {
      x = right + 4;
      y = bottom + 4;
    } else if (w < TRASH_ICON_SIZE + 12) {
      // Narrow but tall (e.g. Temporal Annotation) -> Outside Right, Inside Bottom
      x = right + 4;
      y = bottom - TRASH_ICON_SIZE - 6;
    } else if (visibleH < TRASH_ICON_SIZE + 12) {
      // Short but wide -> Inside Right, Outside Bottom
  ...
  function computeHandles(rectPx) {
    ...
    // Clamp to visible vertical bounds so handles are always reachable
    try {
      const { imageHeight } = getMapping();
      top = Math.max(0, top);
      bottom = Math.min(imageHeight, bottom);
      ...
      if (!isOverBetterTarget) {
        if (pointToRectEdgeDistance(localX, localY, rectPx) === 0) {
          ...
          try {
            ...
            editSession.dragOriginFreq = Math.max(yminHz, Math.min(ymaxHz, ymaxHz - (originY / imageHeight) * (ymaxHz - yminHz)));
  ...
  function onEditPointerMove(ev) {
    ...
    const nyq = globalThis._spectroOriginalNyquist || (globalThis._spectroSampleRate ? globalThis._spectroSampleRate / 2 : 22050);
    const freqAtY = Math.max(yminHz, Math.min(nyq, ymaxHz - (localY / Math.max(1, imageHeight)) * (ymaxHz - yminHz)));
    const w = editSession.working;
    ...
    if (editSession.activeHandle === 'move') {
      ...
      try {
        ...
        if (typeof editSession.dragOriginFreq === 'number') {
          ...
          deltaFreq = ((originYClamped - curYClamped) / Math.max(1, imageHeight)) * (ymaxHz - yminHz);
        } else {
          deltaFreq = 0;
        }
  ...
  function syncToolbarButtons() {
    ...
    const canAction = (count > 0) && !modalOpen;

    let isTemporal = false;
    try { const s = JSON.parse(localStorage.getItem('spectrolipi.settings.v1') || '{}'); isTemporal = s.annotationMode === 'temporal'; } catch(e){}
    const canScc = (count === 1) && !isTemporal && !modalOpen;
  ...
  window.addEventListener('annotation-mode-changed', () => setTimeout(syncToolbarButtons, 0));
  ...
  window.addEventListener('modal-toggled', () => setTimeout(syncToolbarButtons, 0));
