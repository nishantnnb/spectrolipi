...
(function(){
  ...
  modal.innerHTML = `
    <div id="filtersHeader" style="cursor:grab; user-select:none; margin:-12px -12px 12px -12px; padding:12px; background:#1a1a1a; border-radius:8px 8px 0 0; border-bottom:1px solid rgba(255,255,255,0.06);"><h3 style="margin:0;font-size:1rem">Filters</h3></div>
  ...
  <div style="display:flex; gap:16px; align-items:flex-end; margin-top:10px;">
      <div style="flex:1;">
      <label style="display:block;color:#ddd;margin-bottom:6px;">Cutoff frequency (Hz)</label>
      <input id="filterCutoff" type="text" inputmode="numeric" maxlength="5" value="1000" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 5);" style="width:100%;padding:6px;background:#111;border:1px solid rgba(255,255,255,0.06);color:#fff;border-radius:4px" />
      </div>
    <div style="flex:0 0 auto; width:80px;">
      <label style="display:block;color:#ddd;margin-bottom:6px;white-space:nowrap;">Roll-off</label>
      <select id="filterRollOff" style="width:100%;padding:6px;background:#111;border:1px solid rgba(255,255,255,0.06);color:#fff;border-radius:4px">
  ...
  <div id="filtersNote" style="font-size:0.85rem;color:#bbb;margin-top:8px">&nbsp;</div>
  ...
  // Make filters modal draggable
  const fHeader = document.getElementById('filtersHeader');
  let fIsDragging = false;
  let fDragOffX = 0, fDragOffY = 0;
  if (fHeader) {
    fHeader.addEventListener('pointerdown', (e) => {
      fIsDragging = true;
      fHeader.style.cursor = 'grabbing';
      const rect = modal.getBoundingClientRect();
      fDragOffX = e.clientX - rect.left;
      fDragOffY = e.clientY - rect.top;
      fHeader.setPointerCapture(e.pointerId);
    });
    fHeader.addEventListener('pointermove', (e) => {
      if (!fIsDragging) return;
      modal.style.left = (e.clientX - fDragOffX + modal.offsetWidth / 2) + 'px';
      modal.style.top = (e.clientY - fDragOffY + modal.offsetHeight / 2) + 'px';
    });
    fHeader.addEventListener('pointerup', (e) => {
      fIsDragging = false;
      fHeader.style.cursor = 'grab';
      fHeader.releasePointerCapture(e.pointerId);
    });
  }
  ...
  function showModal(){ 
    const noteEl = $('filtersNote');
    if (noteEl) noteEl.innerHTML = '&nbsp;';
    modal.style.display = 'block'; try { window.dispatchEvent(new CustomEvent('modal-toggled')); window.updateCutButtonEnabled && window.updateCutButtonEnabled(); } catch(e){} 
  }
  function hideModal(){ modal.style.display = 'none'; try { window.dispatchEvent(new CustomEvent('modal-toggled')); window.updateCutButtonEnabled && window.updateCutButtonEnabled(); } catch(e){} }
  ...
  const SUPPORTED_SLOPES = Object.freeze([3, 6, 12, 24, 36, 48]);
  ...
  applyBtn.addEventListener('click', async (ev)=>{
    ...
    try {
      ...
      try { window.__spectroWait && window.__spectroWait.show({ etaText: 'Applying filter...', titleText: 'Applying filter', bodyText: 'Processing audio and updating visuals. Please wait...' }); } catch(e){}
      ...
      try {
        ...
        if (sel) {
          ...
          // whole-file edit: recompute spectra from the (modified) audio buffer and rebuild the image.
            try {
              if (prevNote) prevNote.textContent = 'Recomputing full spectrogram...';
  ...
  if (silenceBtn) {
    ...
    silenceModal.innerHTML = `
      <div id="silenceHeader" style="cursor:grab; user-select:none; margin:-12px -12px 12px -12px; padding:12px; background:#1a1a1a; border-radius:8px 8px 0 0; border-bottom:1px solid rgba(255,255,255,0.06);"><h3 style="margin:0;font-size:1rem">Silence</h3></div>
      <div style="font-size:0.85rem;color:#bbb;margin-bottom:12px">Selection length: <span id="silenceSelectionLength">0.000 s</span></div>
      <label style="display:flex;align-items:center;gap:6px;font-size:0.9rem;color:#ddd;margin-bottom:8px;cursor:pointer;">
        <input type="radio" name="silenceMode" value="replace" checked> Replace the selected time span with silence.
      </label>
      <label style="display:flex;align-items:center;gap:6px;font-size:0.9rem;color:#ddd;cursor:pointer;">
        <input type="radio" name="silenceMode" value="insert"> Insert silence
        <input id="silenceDuration" type="number" value="1.000" step="0.05" style="width:64px;padding:4px;background:#111;border:1px solid rgba(255,255,255,0.06);color:#fff;border-radius:4px;text-align:center"> sec.
      </label>
      <div id="silenceNote" style="font-size:0.85rem;color:#bbb;margin-top:8px">&nbsp;</div>
      <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:12px">
    ...
    // Make silence modal draggable
    const sHeader = document.getElementById('silenceHeader');
    let sIsDragging = false;
    let sDragOffX = 0, sDragOffY = 0;
    if (sHeader) {
      sHeader.addEventListener('pointerdown', (e) => {
        sIsDragging = true;
        sHeader.style.cursor = 'grabbing';
        const rect = silenceModal.getBoundingClientRect();
        sDragOffX = e.clientX - rect.left;
        sDragOffY = e.clientY - rect.top;
        sHeader.setPointerCapture(e.pointerId);
      });
      sHeader.addEventListener('pointermove', (e) => {
        if (!sIsDragging) return;
        silenceModal.style.left = (e.clientX - sDragOffX + silenceModal.offsetWidth / 2) + 'px';
        silenceModal.style.top = (e.clientY - sDragOffY + silenceModal.offsetHeight / 2) + 'px';
      });
      sHeader.addEventListener('pointerup', (e) => {
        sIsDragging = false;
        sHeader.style.cursor = 'grab';
        sHeader.releasePointerCapture(e.pointerId);
      });
    }
    ...
    function showSilenceModal(sel){
      ...
      if (silenceNote) silenceNote.innerHTML = '&nbsp;';
      ...
      try { window.dispatchEvent(new CustomEvent('modal-toggled')); window.updateCutButtonEnabled && window.updateCutButtonEnabled(); } catch(e){}
    }
    ...
    if (silenceApply) {
      silenceApply.addEventListener('click', async (ev)=>{
        ...
        const mode = (document.querySelector('input[name="silenceMode"]:checked') || {}).value || 'replace';
        let silenceSec = 0;
        if (mode === 'replace') {
          silenceSec = sel.end - sel.start;
        } else {
          silenceSec = Number(silenceInput ? silenceInput.value : 1);
        }
        silenceSec = clampSilenceDuration(silenceSec);
        if (!isFinite(silenceSec) || silenceSec < 0) {
          alert('Invalid silence duration.');
          return;
        }
        ...
        try {
          ...
          let keepStartSample = startSample;
          let keepEndSample = mode === 'insert' ? startSample : endSample;
          
          const newTotalSamples = keepStartSample + insertSamples + (totalLen - keepEndSample);
          const newBuf = tempCtx.createBuffer(ch, Math.max(1, newTotalSamples), sr);
          for (let c=0;c<ch;c++){
            const src = audioBuf.getChannelData(c);
            const dst = newBuf.getChannelData(c);
            if (keepStartSample > 0) dst.set(src.subarray(0, keepStartSample), 0);
            if (keepEndSample < totalLen) dst.set(src.subarray(keepEndSample), keepStartSample + insertSamples);
          }
          try { if (typeof tempCtx.close === 'function') await tempCtx.close(); } catch(e){}
          ...
          let frameEnd = mode === 'insert' ? frameStart : Math.max(frameStart + 1, Math.ceil(sel.end * fps));
          frameEnd = Math.min(Math.max(frameStart, frameEnd), Math.max(frameStart, totalFrames));
          ...
          try { if (typeof window.adjustAnnotationsForSilence === 'function') await window.adjustAnnotationsForSilence(sel.start, mode === 'insert' ? sel.start : sel.end, silenceSec); } catch(e){ console.warn('adjustAnnotationsForSilence failed', e); }
          ...
          if (mode === 'insert') {
            try { globalThis._spectroCurrentSelection = { start: sel.start + silenceSec, end: sel.end + silenceSec }; } catch(e){}
          } else {
