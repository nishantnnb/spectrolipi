...
(function () {
  ...
  function clientToTimeAndFreq_local(clientX, clientY) {
    ...
    const freqHz = Math.max(yminHz, Math.min(ymaxHz, ymaxHz - t * (ymaxHz - yminHz)));
  ...
  function renderAllAnnotations() {
    ...
    const drawnBoxes = [];
    const COLORS = [
      { fill: 'rgba(0,150,255,0.12)', stroke: 'rgba(0,150,255,0.95)' },    // Blue (default)
      { fill: 'rgba(76,175,80,0.15)', stroke: 'rgba(76,175,80,0.95)' },    // Green
      { fill: 'rgba(156,39,176,0.15)', stroke: 'rgba(156,39,176,0.95)' },  // Purple
      { fill: 'rgba(255,152,0,0.15)', stroke: 'rgba(255,152,0,0.95)' },    // Orange
      { fill: 'rgba(244,67,54,0.15)', stroke: 'rgba(244,67,54,0.95)' }     // Red
    ];
    ...
    rows.forEach(a => {
      ...
      for (const box of drawnBoxes) {
        const overlapTime = Math.max(0, Math.min(endClamped, box.endClamped) - Math.max(beginClamped, box.beginClamped));
        const overlapFreq = Math.max(0, Math.min(high, box.high) - Math.max(low, box.low));
        if (overlapTime > 0 && overlapFreq > 0) {
          usedColors.add(box.colorIndex);
        }
      }
      ...
      drawnBoxes.push({ beginClamped, endClamped, low, high, colorIndex });
      ...
      const t1 = (ymaxHz - high) / Math.max(1, ymaxHz - yminHz);
      const t2 = (ymaxHz - low) / Math.max(1, ymaxHz - yminHz);
      ...
      drawBoxOnOverlay(x1, y1, x2, y2, { fill: theme.fill, stroke: theme.stroke, dashed: false });
    });
  ...
  function onPointerDown(ev) {
    ...
    const vwRect = viewportWrapper.getBoundingClientRect();
    const yInCanvas = ev.clientY - vwRect.top;
    ...
    pending = {
      ...
      startFreq: isTemporal ? nyq : start.freqHz,
      currentTime: start.timeSec,
      currentFreq: isTemporal ? 0 : start.freqHz,
